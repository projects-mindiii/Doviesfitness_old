...
class FeedAdapter(context: Context, feedList: ArrayList<AllOtherThenFeatured>, feedListOnClick: FeedListOnClick) :
    androidx.recyclerview.widget.RecyclerView.Adapter<androidx.recyclerview.widget.RecyclerView.ViewHolder>() {
    private lateinit var description: SpannableString
    var mContext: Context
    var feedList: ArrayList<AllOtherThenFeatured>
    var feedListOnClick: FeedListOnClick
  ...
  /* lateinit var featuredDatatemp: AllOtherThenFeatured
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): androidx.recyclerview.widget.RecyclerView.ViewHolder {
        when (viewType) {
            VIEWTYPE_ITEM -> {
                return ViewHolder(LayoutInflater.from(mContext).inflate(R.layout.feed_view, parent, false))
            }
            else -> {
                return FooterLoader(
                    LayoutInflater.from(mContext).inflate(R.layout.pagination_item_loader, parent, false)
                )
    ...
    override fun onBindViewHolder(rvHolder: androidx.recyclerview.widget.RecyclerView.ViewHolder, p1: Int) {
        ...
        val featuredData: AllOtherThenFeatured = feedList.get(p1)
        ...
        Glide.with(holder.ivFeed.context).load(featuredData.news_image).placeholder(R.color.colorBlack).into(holder.ivFeed)
        ...
        holder.ivFav.setOnClickListener {
            feedListOnClick.ivFavOnclick(featuredData, holder.adapterPosition)
        }
    ...
    fun startStopHandler(data:AllOtherThenFeatured,pos:Int){
        ...
        input_finish_checker = Runnable {
            if (System.currentTimeMillis() > last_text_edit[0] + delay - 500) {
                Log.d("in other thread","in other thread...."+data.news_like_status+".......pos..."+pos)
...
*/
    ...
    fun changeFontFamily(featuredData:AllOtherThenFeatured,adapterPosition:Int, tvDescription: TextView): SpannableStringBuilder {
        val builder = SpannableStringBuilder()
        val name = SpannableString(featuredData.news_creator_name)
        name.setSpan(StyleSpan(Typeface.BOLD), 0, name.length, 0)
        builder.append(name)
        builder.append(" ")
        if ((builder.length + featuredData.news_description.length) > 150) {

            if(featuredData.news_description.length > 150){
                description = SpannableString(featuredData.news_description.substring(0, 150))
            }else{
                description = SpannableString(featuredData.news_description.substring(0, featuredData.news_description.length -1))
            }
            ...
            val description = SpannableString(featuredData.news_description)
            builder.append(description)
    ...
    override fun getItemId(position: Int): Long {
        val featuredData: AllOtherThenFeatured = feedList.get(position)
    ...
    interface FeedListOnClick {
        fun tvCommentsOnclick(featuredData: AllOtherThenFeatured, pos : Int)
        fun ivLikeOnclick(featuredData: AllOtherThenFeatured, pos: Int, ivHeart: ImageView)
        fun ivShareOnclick(featuredData: AllOtherThenFeatured, pos : Int)
        fun ivFavOnclick(featuredData: AllOtherThenFeatured, pos : Int)
        fun tvDescriptioOnclick(featuredData: AllOtherThenFeatured, pos : Int)
    }
}
