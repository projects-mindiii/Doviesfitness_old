...
class MyDietPlanFragment : BaseFragment(), MyDietPlanAdapterExample.MyDietPlanLIstener,
    ...
    override fun onCreateView(
        ...
        binding =
            DataBindingUtil.inflate(inflater, R.layout.fragment_my_diet_plan, container, false)
        initialization()
        return binding.root
    }
    ...
    private fun initialization() {
        ...
        //binding.myDpRv.addItemDecoration(ItemOffsetDecoration(mContext, R.dimen._10sdp))
        ...
        if (arguments != null) {
            dataValue = arguments!!.getInt("data")
            if (dataValue == 1) {
        ...
        adapter = MyDietPlanAdapterExample(mContext, myDietPlanList, this)
...
//                                binding.noRecordFound.visibility = View.GONE
    ...
    override fun getDietPlanInfo(data: MyDietPlan.Data, position: Int, whichClick: String) {
        ...
        if (whichClick.equals("1")) {
            DeleteMyDietPlan.newInstance(this, data, position).show(childFragmentManager)
        }
    }
    ...
    override fun textOnClick1(type: String, id: MyDietPlan.Data, pos: Int) {
        ...
        if (type.equals(getString(R.string.delete))) {
            showDeleteDialog(mContext as Activity, id.diet_plan_id, pos)
        } else {
            //Delete Dialog
            Constant.showCustomToast(mContext, type)
        }
    }
